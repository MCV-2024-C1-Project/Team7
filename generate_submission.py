...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Input for submission generation.")
...
def generate_submission_top10(results, paintings_per_image):
    """
    Gets the top 1 bbdd image for each query, taking into account
    whether there were one or two paintings in the image.
    ...
    for num_paintings in paintings_per_image:
        if num_paintings == 1:
            submission.append([list(results[i][0:10])])
            i += 1
        elif num_paintings == 2:
            submission.append([list(results[i][0:10]), list(results[i+1][0:10])])
            i += 2
...
def main():
	...
	# Call the main function with the best method and parameters
    results = get_predictions(query_dir, bbdd_dir,
                          ...
                          matching_params=[dict(algorithm=6, table_number=6, key_size=12, multi_probe_level=1), 
                                           dict(checks=50), 2, 0.7],
                          unknown_painting_threshold=0.5062,
                          cache_segmented=False,
                          cache_denoised=False)
	
	# Load "paintings per image" support file to group the paintings in 1 or 2
    with open("data/paintings_per_image.pkl", "rb") as f:
        paintings_per_image = pickle.load(f)
	
	# Generate the submission structure (list of lists of lists)
    submission = generate_submission_top10(results, paintings_per_image)
	
	# Save the submission to pkl
